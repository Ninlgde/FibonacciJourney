...
我们换一种思路，假设你已经站在第十阶了，请问你是怎么到第十阶的呢？
...
好了，前面说了那么多，终于引入我们的主题了斐波那契数列的实现。
...
看看慢在哪里！看一下函数调用过程，下面展示我们看看计算5的调用过程

![2^n](images/fib02.jpeg)

我的乖乖，算一个数，每次要分裂两次，而且每一项都需要重复去算，时间复杂度是O(2^n)，所以算个45要这么慢！
...
emmmm，其实也不是特别大 64*2000/1024=125k的内存，完全oxxk

可能有些人对如何计算时间空间复杂度不是很懂，简单的理解为如果你的输入是n，那么到得出结果，你要进行的步骤所消耗的时间就是时间复杂度，消耗的空间就是空间复杂度。这个大家如果感兴趣可以去看《算法导论》，或者Google一下。
...
都是O(n)啊，为啥是变超跑，最后所有的解决方案都说完了，我们比较一下

哎，等等，为啥是所有解决的方案，难道这3个还不能令你满意吗？
...
好了，其实没关系，我既然敢把这个公式列出来，那比如有妙计，看我的大杀器！
...
talk is cheap, show me the code 看起来比较抽象，直接看代码吧。
```go
...
明明我们只需要算一个数，缺额外计算了这么多次。作为精益求精的工程师能忍吗？不能忍！
...
如图，下面的练习1.13就是我所说的证明题，解题过程就在上面。精髓在”易证“两字上，至于我为啥不写全，其实和费马不证明费马大定律是一个原因：纸不够了。
...
因为也是计算n次方我们还是用了快速幂算法。但是细心的同学会发现，我还写了一个buildin的方式来对比
...
|BenchmarkFibFormulaBuildIn-12 | 72.6 ns/op| 70.7 ns/op|
...
- 矩阵法O(log(n))，快，需要数学知识，能飞的里面是弟中弟
- buildin公式法O(log(n))，不是最快，因为看了math.Pow的实现会发现里面有大量的别的计算
  - 这个故事告诉我们不要盲目相信，系统提供的就是最好的，具体问题具体分析
...
理解了如何计算时空负责度，会对你如何写优秀的代码有很大的帮助
